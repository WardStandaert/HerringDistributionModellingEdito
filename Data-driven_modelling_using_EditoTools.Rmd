...
knitr::opts_knit$set(root.dir = "/home/onyxia/work/EDITO_CALCULATIONS/")
...
#this takes about 13 mins
model_fit <- ENMeval::ENMevaluate(occs = df_occ_bg_env %>%                                    
                                    filter(presence == 1) %>% 
                                    dplyr::select(Longitude, Latitude, thetao, so, zooc, phyc),
                                  bg = df_occ_bg_env %>% 
                                    filter(presence == 0) %>% 
                                    dplyr::select(Longitude, Latitude, thetao, so, zooc, phyc),
...
vs <- c("thetao", "so", "phyc", "zooc")

#variables names for the plot
name_key <- data.frame(old = c("thetao", "so", "phyc", "zooc"),
...
grid.arrange(grobs = list(thetao, so + xlab(""), 
                          phyc, zooc + xlab("")))
```
...
variables <- c("thetao", "so", "phyc", "zooc")
